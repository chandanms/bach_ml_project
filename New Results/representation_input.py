...
def representation_input(notes_list):
    ...
    min_p = 2 * math.log(2 ** (min(list(filter(lambda a: a != 0, notes_list))) - 69) / 12 * 440, 2)
    max_p = 2 * math.log(2 ** (max(list(filter(lambda a: a != 0, notes_list))) - 69) / 12 * 440, 2)
    ...
    for midi_note in notes_list:
        ...
        if midi_note != 0:
            ...
            y = [0, pitch, chroma_x, chroma_y, c5_x, c5_y]
            # Add y vector to representation
            representation.append(y)

        # if we the same note
        else:
            # Midi note is nul add the following vector
            y = [1, 0, 0, 0, 0, 0]
            representation.append(y)
